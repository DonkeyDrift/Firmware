...
## 简介
本文件聚焦于MUS4项目的核心数据结构，尤其是struct_message结构体的设计理念与字段语义，并系统阐述其在系统中的流转过程与相互关系。重点覆盖以下方面：
- struct_message的字段设计：throttle（油门值-100~100）、steering（转向值-100~100）、mode（驾驶模式）、park（停车标志）的作用与取值范围
- 各数据实例的用途：rc_data（RC接收机处理后数据）、pilot_data（上位机指令）、car_output（最终输出控制量）、esp_now_data（ESP-NOW通信数据）
- 数据流在系统中的传递路径、一致性与完整性保障机制

## 项目结构
MUS4项目采用“单文件主程序+文档”的组织方式，核心逻辑集中在mus4.ino中，配套文档位于Doc目录，涵盖架构、硬件引脚定义与开发笔记等。

```mermaid
graph TB
A["mus4.ino<br/>主程序与核心数据结构"] --> B["Doc/Arch/architecture.md<br/>系统架构与流程"]
A --> C["Doc/README/DevNote.md<br/>开发笔记与常量说明"]
A --> D["Doc/Hardware/pin_definitions.md<br/>硬件引脚与连接关系"]
...
- struct_message结构体：统一承载控制指令与状态，包含throttle、steering、mode、park四个字段，用于在不同数据源之间传递控制意图与系统状态。
...
## 架构总览
MUS4的控制回路由“输入采集—解析—融合—映射—执行”构成，核心数据结构贯穿其中，确保数据在不同阶段的一致性与完整性。
...
ISR-->>Fusion : rc_data(标准化值)
Parser-->>Fusion : pilot_data(上位机指令)
Fusion->>Fusion : 根据mode/park融合控制量
Fusion-->>Mapper : car_output(-100~100)
...
章节来源
- [mus4.ino](file://mus4/mus4.ino#L460-L467)
...
章节来源
- [mus4.ino](file://mus4/mus4.ino#L468-L472)
- [mus4.ino](file://mus4/mus4.ino#L1152-L1290)
- [DevNote.md](file://mus4/Doc/README/DevNote.md#L88-L92)
...
## 依赖关系分析
- 数据结构依赖
  - struct_message被rc_data、pilot_data、car_output、esp_now_data四者共享，形成统一的数据契约
- 控制流依赖
...
```mermaid
graph TB
SM["struct_message<br/>统一数据结构"] --> RC["rc_data<br/>RC接收机数据"]
...
## 结论
MUS4通过struct_message统一了控制数据的表达方式，配合RC中断、串口解析、模式/停车融合与PWM映射，构建了清晰、可扩展且具备安全保护的控制闭环。该设计在保证数据一致性与完整性的同时，兼顾实时性与可维护性，为后续功能扩展（如ESP-NOW通信）提供了良好的基础。