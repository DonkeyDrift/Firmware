...
<button id="helpFab" class="helpFab" onclick="openHelpModal()" aria-label="功能说明">?</button>
<div id="helpOverlay" class="helpOverlay" onclick="closeHelpModal()"></div>
<div id="helpModal" class="helpModal" role="dialog" aria-modal="true" aria-labelledby="helpTitle"><div class="helpHead"><h2 id="helpTitle">功能说明</h2><button class="helpClose" onclick="closeHelpModal()" aria-label="关闭功能说明">×</button></div><ul class="helpList"><li>状态卡片：查看模式、Park、OTA、连接状态</li><li>Network：查看 AP/STA IP，配置 Wi-Fi</li><li>Diagnostics：运行测试、回归、维护命令</li><li>Serial Log：查看设备日志和命令反馈</li><li>Tub JSON：记录并下载遥测样本</li><li>OTA / DEV：固件更新与开发模式开关</li></ul></div>
...
function openHelpModal(){helpOverlay.classList.add('show');helpModal.classList.add('show')}
