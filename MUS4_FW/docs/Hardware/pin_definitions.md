...
*   **软件配置**：
    *   使用 `FastLED` 库驱动。
    *   **颜色定义**：
        *   **红/绿/蓝三通道齐亮（白色）常亮 3 秒**：上电自检，随后进入状态指示。
        *   **绿色**：手动模式 (Manual)。
        *   **黄色**：半自动模式 (Semi-Auto)。
        *   **蓝色**：全自动模式 (Full-Auto)。
        *   **空闲灯色可配置（Web Console 多选）**：手动模式 + 停车 (Park) 时按勾选的颜色交替闪烁——缺省红/绿/蓝三色全选，勾选两色双色交替、单色亮灭闪烁（亮灭各 250ms）、全不选熄灭；选择 NVS 持久化。
        *   **模式色/红色交替闪烁**：半自动或全自动模式 + 停车 (Park)。
        *   **灭/红/绿/蓝随机乱闪**：OTA 固件传输期间（HTTP `/update` 与 ArduinoOTA 两条通道）的故障灯效——随机颜色、随机间隔（30-120ms）；传输中止/失败立即归还正常状态指示，传输成功则经 RTC 标记跨重启延续，直到开机蜂鸣器旋律播完（空闲 800ms 宽限）才恢复。
