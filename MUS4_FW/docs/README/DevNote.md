...
- **Serial1 遥测回传帧**：`T:S\n`
  - `T` 代表 Throttle（油门值）
  - `S` 代表 Steering（转向值）
  - 仅在 **MANUAL 模式**下回传；ASSIST / AUTO 模式下关闭 TX 遥测，避免与上位机控制帧重复
  - 回传最近一次从 Serial1 接收到的控制指令数值
  - 刷新率约 60 Hz（与常见上位机帧率对齐）
  - 范围：−100 ~ 100
  - 示例：`T10:S20\n`
...
- MPU6050 IMU 传感器
- INA219 电压电流监测
- I2C 总线初始化（已验证正确引脚：SDA=21, SCL=22）
