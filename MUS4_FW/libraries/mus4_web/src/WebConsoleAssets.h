...
<script>
async function getLauncherHostIp(){try{const r=await fetch('/api/status');const t=await r.text();const m=t.match(/host_ip=(\S+)/);if(m&&m[1])return m[1];}catch(e){}return '192.168.3.41';}
async function enterDonkeyLauncher(){const ip=await getLauncherHostIp();window.open('http://'+ip+':8090/','_blank');}
async function enterDonkeyDrifter(){const ip=await getLauncherHostIp();location.href='http://'+ip+':8090/api/launch/drive';}
