...
// CC 内嵌设置视图（embedded+settings）：向 DD 父页面 postMessage 上报文档完整高度，父页面据此
// 把 iframe 拉到与内容等高，实现整页一滑到底（iframe 内部不再单独滚动）。跨域父页面无法直接
// 量这里，只能走 postMessage；两个懒加载子 iframe（/drift、/judge）加载后会撑高页面，靠
// ResizeObserver 持续捕获高度变化。独立打开（无 embedded）时不调用本函数，行为完全不变。
function initEmbedHeightReporter(){var post=function(){try{var h=Math.max(document.documentElement?document.documentElement.scrollHeight:0,document.body?document.body.scrollHeight:0);if(h>0&&window.parent&&window.parent!==window)window.parent.postMessage({type:'dd-embed-height',height:h},'*')}catch(e){}};post();window.addEventListener('load',post);window.addEventListener('resize',post);if(window.ResizeObserver){if(document.body)new ResizeObserver(post).observe(document.body);if(document.documentElement)new ResizeObserver(post).observe(document.documentElement)}}
