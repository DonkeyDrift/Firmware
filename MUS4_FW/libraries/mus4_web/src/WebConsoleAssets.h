...
<script>
var _launcherIp='192.168.3.41';async function _fetchLauncherIp(){try{const r=await fetch('/api/status');const t=await r.text();const m=t.match(/host_ip=(\S+)/);if(m&&m[1])_launcherIp=m[1];}catch(e){}}_fetchLauncherIp();
function enterDonkeyLauncher(){window.open('http://'+_launcherIp+':8090/','_blank');}
function enterDonkeyDrifter(){window.open('http://'+_launcherIp+':8090/launch/drive','_blank');}
