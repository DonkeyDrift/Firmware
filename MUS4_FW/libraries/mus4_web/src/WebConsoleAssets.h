...
<div id="driftCard" class="stateCard"><span class="tunePair"><a href="/drift">Tune</a><span class="stateDot"></span></span><div class="stateHead" data-i18n="state.drift">Drift</div><div class="stateValue" id="driftValue">--</div><div class="stateSub" id="driftSub">waiting</div><div class="driftBar"><i id="driftNeedle"></i></div></div>
...
<div id="reconnectOverlay" class="reconnectOverlay"><div class="reconnectBox"><div class="reconnectSpinner"></div><div class="reconnectText"><span data-i18n="reconnect.title">连接已断开</span><br><span data-i18n="reconnect.body">正在尝试重新连接...</span></div></div></div>
<script>
var _launcherIp='192.168.3.41';async function _fetchLauncherIp(){try{const r=await fetch('/api/status');const t=await r.text();const m=t.match(/host_ip=(\S+)/);if(m&&m[1]){_launcherIp=m[1];document.getElementById('enterDonkeyBtn').href='http://'+_launcherIp+':8090/';document.getElementById('enterDonkeyDrifterBtn').href='http://'+_launcherIp+':8090/#drive';}}catch(e){}}_fetchLauncherIp();
...
const LANG_STORAGE_KEY='mus4.ui.lang';const THEME_STORAGE_KEY='mus4.ui.theme';
...
I18N.zh['wifi.hostStatus.idle']='等待...';
I18N.en['wifi.hostStatus.idle']='Waiting...';
...
let uiMuted=false;
let uiLedBlinkMask=7;
const LED_BLINK_TAB_COLORS={1:'#ff6b6b',2:'#39d98a',4:'#5cc8ff'};
let uiTheme='auto';
...
function applyLanguage(lang){uiLang=normalizeLanguage(lang);document.documentElement.lang=uiLang;document.querySelectorAll('[data-i18n]').forEach(e=>{const v=t(e.dataset.i18n);if(v)e.textContent=v});document.querySelectorAll('[data-i18n-placeholder]').forEach(e=>{const v=t(e.dataset.i18nPlaceholder);if(v)e.placeholder=v});document.querySelectorAll('[data-i18n-aria]').forEach(e=>{const v=t(e.dataset.i18nAria);if(v)e.setAttribute('aria-label',v)});document.querySelectorAll('[data-i18n-title]').forEach(e=>{const v=t(e.dataset.i18nTitle);if(v)e.title=v});document.querySelectorAll('button[data-lang]').forEach(b=>b.classList.toggle('active',b.dataset.lang===uiLang));refreshDynamicLabels();refreshJoystickCalStatus()}
function setLanguage(lang){uiLang=normalizeLanguage(lang);writeStoredLanguage(uiLang);applyLanguage(uiLang);fetch('/api/language?lang='+uiLang,{method:'POST'}).catch(()=>{});closeLanguageMenu()}
async function initLanguage(){let lang='';try{const r=await fetch('/api/language',{cache:'no-store'});if(r.ok){const j=await r.json();if(j&&(j.lang==='zh'||j.lang==='en')){lang=normalizeLanguage(j.lang);writeStoredLanguage(lang)}}}catch(e){}if(!lang)lang=readStoredLanguage();applyLanguage(lang)}
...
function renderLedBlinkTabs(){const t=document.getElementById('ledBlinkTabs');if(!t)return;const bs=[...t.querySelectorAll('button[data-color]')];bs.forEach((b,i)=>{const on=(uiLedBlinkMask&parseInt(b.dataset.color,10))!==0;b.classList.toggle('active',on);const prev=i>0&&(uiLedBlinkMask&parseInt(bs[i-1].dataset.color,10))!==0;const next=i<bs.length-1&&(uiLedBlinkMask&parseInt(bs[i+1].dataset.color,10))!==0;b.style.borderRadius=on?(prev?0:999)+'px '+(next?0:999)+'px '+(next?0:999)+'px '+(prev?0:999)+'px':'';const sh=[],c=LED_BLINK_TAB_COLORS[b.dataset.color]||'#5cc8ff';if(on&&prev)sh.push('-2px 0 0 '+c);if(on&&next)sh.push('2px 0 0 '+c);b.style.boxShadow=sh.join(',')})}
async function initLedBlink(){try{const r=await fetch('/api/led-blink',{cache:'no-store'});if(!r.ok)return;const j=await r.json();if(typeof j.colors==='number'&&j.colors>=0&&j.colors<=7){uiLedBlinkMask=j.colors;renderLedBlinkTabs()}}catch(e){}}
async function toggleLedBlinkColor(bit){const next=uiLedBlinkMask^bit;try{const r=await fetch('/api/led-blink',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'},body:'colors='+next});if(!r.ok)return;const j=await r.json();if(typeof j.colors==='number'&&j.colors>=0&&j.colors<=7){uiLedBlinkMask=j.colors;renderLedBlinkTabs()}}catch(e){}}
function readStoredTheme(){try{return localStorage.getItem(THEME_STORAGE_KEY)||'auto'}catch(e){return 'auto'}}
function writeStoredTheme(theme){try{localStorage.setItem(THEME_STORAGE_KEY,theme)}catch(e){}}
function renderThemeTabs(){document.querySelectorAll('#themeTabs button[data-theme]').forEach(b=>b.classList.toggle('active',b.dataset.theme===uiTheme))}
function setTheme(theme){uiTheme=theme;writeStoredTheme(uiTheme);renderThemeTabs()}
function initTheme(){uiTheme=readStoredTheme();renderThemeTabs()}
...
function updateApPreview(){apSsid.value=apSsid.value.replace(/[^A-Za-z0-9]/g,'');const p=apSsid.value.trim();apPreview.textContent=p?t('wifi.apPreview')+p+'-ESP':''}async function openWifiApModal(){apNotice.textContent=t('wifi.apNotice');apSaveBtn.disabled=false;apSaveBtn.textContent=t('wifi.saveRestartAp');await refreshWifiAp();const s=apSsid.value;if(s&&s.endsWith('-ESP'))apSsid.value=s.slice(0,-4);updateApPreview();wifiApModal.classList.add('show')}
function closeWifiApModal(){if(apSaving)return;wifiApModal.classList.remove('show')}
async function saveWifiAp(){if(apSaving)return;const v=apSsid.value.trim();if(!v||!/^([A-Za-z0-9]{1,6})$/.test(v)){apNotice.textContent=t('wifi.apInvalid');return}try{apSaving=true;apSaveBtn.disabled=true;apSaveBtn.textContent=t('wifi.apSaving');apNotice.textContent=t('wifi.apSavingNotice');const body=new URLSearchParams();body.set('ssid',v);const r=await fetch('/api/wifi-ap',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body});if(!r.ok){const txt=await r.text();showCommandError(txt);if(txt.includes('invalid_ssid'))apNotice.textContent=t('wifi.apInvalid');else apNotice.textContent=t('wifi.apSaveFailed');throw new Error(txt)}showToast(t('wifi.apSaved'),true);wifiApModal.classList.remove('show');refreshStatus()}catch(e){line('wifi ap save error: '+e)}finally{apSaving=false;apSaveBtn.disabled=false;apSaveBtn.textContent=t('wifi.saveRestartAp')}}
...
<div class="muted"><a href="/" data-i18n="judge.backLink">返回 Drifter Console</a></div>
</div>
</div>
</div>
<div class="hero">
...
const I18N={zh:{},en:{}};
I18N.zh['judge.backLink']='返回 Drifter Console';I18N.en['judge.backLink']='Back to Drifter Console';
...
I18N.zh['judge.dimDesc']='监控优先，但保留实时总分与 6 维条';I18N.en['judge.dimDesc']='Monitor-first, with live total score and all 6 dimension bars kept';
...
I18N.zh['judge.section.thresholdsDesc']='先调判定边界，再看评分手感。';I18N.en['judge.section.thresholdsDesc']='Tune the decision boundaries first, then the scoring feel.';
I18N.zh['judge.section.scoring']='评分参数';I18N.en['judge.section.scoring']='Scoring Parameters';
I18N.zh['judge.section.scoringDesc']='这一组控制评分灵敏度，数值越大通常越敏感。';I18N.en['judge.section.scoringDesc']='This group controls scoring sensitivity; larger values are usually more sensitive.';
...
I18N.zh['judge.score.reasonInitial']='拖分原因：拖分分析中，继续保持当前动作。';I18N.en['judge.score.reasonInitial']='Reason: Analysis in progress; keep the current action going.';
I18N.zh['judge.reason.analyzing']='拖分分析中，继续保持当前动作。';I18N.en['judge.reason.analyzing']='Analysis in progress; keep the current action going.';
...
function writeStoredLanguage(lang){try{localStorage.setItem(LANG_STORAGE_KEY,lang)}catch(e){}}
function t(key){return (I18N[uiLang]&&I18N[uiLang][key])||I18N.zh[key]||key}
function applyLanguage(lang){uiLang=normalizeLanguage(lang);document.documentElement.lang=uiLang;document.querySelectorAll('[data-i18n]').forEach(e=>{const v=t(e.dataset.i18n);if(v)e.textContent=v});document.querySelectorAll('[data-i18n-placeholder]').forEach(e=>{const v=t(e.dataset.i18nPlaceholder);if(v)e.placeholder=v});document.querySelectorAll('[data-i18n-aria]').forEach(e=>{const v=t(e.dataset.i18nAria);if(v)e.setAttribute('aria-label',v)});document.querySelectorAll('[data-i18n-title]').forEach(e=>{const v=t(e.dataset.i18nTitle);if(v)e.title=v});refreshDynamicLabels()}
async function initLanguage(){let lang='';try{const r=await fetch('/api/language',{cache:'no-store'});if(r.ok){const j=await r.json();if(j&&(j.lang==='zh'||j.lang==='en')){lang=normalizeLanguage(j.lang);writeStoredLanguage(lang)}}}catch(e){}if(!lang)lang=readStoredLanguage();applyLanguage(lang)}
...
function setJudgeConfigStatus(key,kind,detail){judgeConfigStatusState={key:key,kind:kind||'',detail:detail||''};judgeConfigStatusEl.textContent=t(key)+(detail||'');judgeConfigStatusEl.style.color=kind==='ok'?'#39d98a':kind==='err'?'#ff7b7b':'#8fa1b5'}
...
<body>
<div class="headerRow"><h1 data-i18n="drift.title">Drift Assist Tuning</h1><span class="version" data-i18n="drift.versionTag">GYRO STEERING / THROTTLE</span><a href="/" data-i18n="drift.backLink">Back to Drifter Console</a></div>
<div class="panel">
<div class="panelHead"><div><div class="label" data-i18n="drift.status.label">Status</div><div class="muted" data-i18n="drift.status.desc">Live drift state from device</div></div><div id="driftStateCurrent" class="muted" data-i18n="drift.state.waiting">waiting...</div></div>
...
<div class="panel">
<div class="panelHead"><div><div class="label" data-i18n="drift.steering.label">Steering Correction</div><div class="muted" data-i18n="drift.steering.desc">Map steering to target yaw rate and apply P+D feedback.</div></div><div id="driftConfigCurrent" class="muted" data-i18n="drift.config.reading">reading device config...</div></div>
<div class="tuneGrid">
...
<div class="tuneSection"><div class="sectionTitle" data-i18n="drift.throttle.title">Throttle Strategy</div><div class="sectionDesc" data-i18n="drift.throttle.desc">Switch between continuous spin throttle and pulsed drift throttle.</div>
<div class="tuneGrid">
...
const LANG_STORAGE_KEY='mus4.ui.lang';
const I18N={zh:{'drift.title':'漂移辅助调参','drift.versionTag':'陀螺仪转向 / 油门','drift.backLink':'返回 Drifter Console','drift.status.label':'状态','drift.status.desc':'来自设备的实时漂移状态','drift.state.waiting':'等待中...','drift.state.enabled':'启用','drift.state.active':'激活','drift.state.yawError':'偏航误差','drift.state.throttleMode':'油门模式','drift.steering.label':'转向修正','drift.steering.desc':'将转向映射为目标偏航角速度并施加 P+D 反馈。','drift.config.reading':'正在读取设备配置...','drift.config.current':'当前设备配置','drift.config.usingDefaults':'使用页面默认值','drift.throttle.title':'油门策略','drift.throttle.desc':'在连续旋转油门与脉冲漂移油门之间切换。','drift.action.save':'保存漂移配置','drift.action.reset':'恢复默认值','drift.status.saveHint':'保存后持久化到设备 NVS。','drift.status.synced':'已同步设备配置','drift.status.loadFailed':'读取配置失败，使用默认值','drift.status.invalidInput':'输入超出允许范围','drift.status.saved':'已保存到设备 NVS','drift.status.saveFailed':'保存失败: ','drift.status.reset':'已恢复默认值并保存到设备','drift.status.resetFailed':'重置失败: ','drift.error.saveFailed':'保存失败','drift.error.resetFailed':'重置失败','drift.value.on':'开','drift.value.off':'关','drift.value.active':'生效中','drift.value.armed':'待命','drift.value.spin':'旋转','drift.value.pulse':'脉冲','drift.value.pass':'直通','drift.state.seqPrefix':'序号 ','drift.state.offline':'离线','drift.field.steeringGyroSign':'转向陀螺仪符号','drift.field.steeringGyroSign.hint':'-1 或 1：翻转转向↔陀螺仪映射方向。','drift.field.maxYawRate':'最大偏航角速度','drift.field.maxYawRate.hint':'满舵时预期的偏航角速度峰值（rad/s）。','drift.field.kp':'Kp','drift.field.kp.hint':'偏航误差的比例增益。','drift.field.kd':'Kd','drift.field.kd.hint':'当前偏航角速度的阻尼增益。','drift.field.maxSteeringCorrection':'最大转向修正量','drift.field.maxSteeringCorrection.hint':'0~1：限制归一化转向偏移的上限。','drift.field.gyroFilterAlpha':'陀螺仪滤波系数','drift.field.gyroFilterAlpha.hint':'0~1：越大响应越快，噪声也越大。','drift.field.spinThreshold':'旋转阈值','drift.field.spinThreshold.hint':'|gyroZ| 超过此值时使用连续油门。','drift.field.steeringThreshold':'转向阈值','drift.field.steeringThreshold.hint':'油门策略介入所需的最小归一化转向量。','drift.field.continuousThrottle':'连续油门','drift.field.continuousThrottle.hint':'0~1：高偏航角速度旋转期间使用的油门。','drift.field.pulseThrottle':'脉冲油门','drift.field.pulseThrottle.hint':'0~1：脉冲导通阶段的峰值油门。','drift.field.pulseFreqHz':'脉冲频率 (Hz)','drift.field.pulseFreqHz.hint':'漂移油门模式下每秒的脉冲次数。','drift.field.pulseDuty':'脉冲占空比','drift.field.pulseDuty.hint':'0~1：每个脉冲的导通时间占比。'},en:{'drift.title':'Drift Assist Tuning','drift.versionTag':'GYRO STEERING / THROTTLE','drift.backLink':'Back to Drifter Console','drift.status.label':'Status','drift.status.desc':'Live drift state from device','drift.state.waiting':'waiting...','drift.state.enabled':'Enabled','drift.state.active':'Active','drift.state.yawError':'Yaw Error','drift.state.throttleMode':'Throttle Mode','drift.steering.label':'Steering Correction','drift.steering.desc':'Map steering to target yaw rate and apply P+D feedback.','drift.config.reading':'reading device config...','drift.config.current':'Current device config','drift.config.usingDefaults':'Using page defaults','drift.throttle.title':'Throttle Strategy','drift.throttle.desc':'Switch between continuous spin throttle and pulsed drift throttle.','drift.action.save':'Save Drift Config','drift.action.reset':'Restore Defaults','drift.status.saveHint':'Save to persist in device NVS.','drift.status.synced':'Synced device config','drift.status.loadFailed':'Failed to read config, using defaults','drift.status.invalidInput':'Input out of allowed range','drift.status.saved':'Saved to device NVS','drift.status.saveFailed':'Save failed: ','drift.status.reset':'Restored defaults and saved to device','drift.status.resetFailed':'Reset failed: ','drift.error.saveFailed':'save_failed','drift.error.resetFailed':'reset_failed','drift.value.on':'ON','drift.value.off':'OFF','drift.value.active':'ACTIVE','drift.value.armed':'ARMED','drift.value.spin':'spin','drift.value.pulse':'pulse','drift.value.pass':'pass','drift.state.seqPrefix':'seq ','drift.state.offline':'offline','drift.field.steeringGyroSign':'Steering Gyro Sign','drift.field.steeringGyroSign.hint':'-1 or 1: flips the steering↔gyro mapping direction.','drift.field.maxYawRate':'Max Yaw Rate','drift.field.maxYawRate.hint':'Expected peak yaw rate at full steering (rad/s).','drift.field.kp':'Kp','drift.field.kp.hint':'Proportional gain on yaw error.','drift.field.kd':'Kd','drift.field.kd.hint':'Damping gain on current yaw rate.','drift.field.maxSteeringCorrection':'Max Steering Correction','drift.field.maxSteeringCorrection.hint':'0~1: caps the normalized steering offset.','drift.field.gyroFilterAlpha':'Gyro Filter Alpha','drift.field.gyroFilterAlpha.hint':'0~1: higher = more responsive, more noise.','drift.field.spinThreshold':'Spin Threshold','drift.field.spinThreshold.hint':'|gyroZ| above this uses continuous throttle.','drift.field.steeringThreshold':'Steering Threshold','drift.field.steeringThreshold.hint':'Min normalized steering before throttle strategy engages.','drift.field.continuousThrottle':'Continuous Throttle','drift.field.continuousThrottle.hint':'0~1: throttle used during high yaw-rate spin.','drift.field.pulseThrottle':'Pulse Throttle','drift.field.pulseThrottle.hint':'0~1: peak throttle during pulse-on phase.','drift.field.pulseFreqHz':'Pulse Freq (Hz)','drift.field.pulseFreqHz.hint':'Pulses per second in drift throttle mode.','drift.field.pulseDuty':'Pulse Duty','drift.field.pulseDuty.hint':'0~1: on-time fraction of each pulse.'}};
let uiLang=readStoredLanguage();
function normalizeLanguage(lang){return lang==='en'?'en':'zh'}
function readStoredLanguage(){try{return normalizeLanguage(localStorage.getItem(LANG_STORAGE_KEY))}catch(e){return 'zh'}}
function writeStoredLanguage(lang){try{localStorage.setItem(LANG_STORAGE_KEY,lang)}catch(e){}}
...
function applyLanguage(lang){uiLang=normalizeLanguage(lang);document.documentElement.lang=uiLang;document.querySelectorAll('[data-i18n]').forEach(e=>{const v=t(e.dataset.i18n);if(v)e.textContent=v});document.querySelectorAll('[data-i18n-placeholder]').forEach(e=>{const v=t(e.dataset.i18nPlaceholder);if(v)e.placeholder=v});document.querySelectorAll('[data-i18n-aria]').forEach(e=>{const v=t(e.dataset.i18nAria);if(v)e.setAttribute('aria-label',v)});document.querySelectorAll('[data-i18n-title]').forEach(e=>{const v=t(e.dataset.i18nTitle);if(v)e.title=v});refreshDynamicLabels()}
async function initLanguage(){let lang='';try{const r=await fetch('/api/language',{cache:'no-store'});if(r.ok){const j=await r.json();if(j&&(j.lang==='zh'||j.lang==='en')){lang=normalizeLanguage(j.lang);writeStoredLanguage(lang)}}}catch(e){}if(!lang)lang=readStoredLanguage();applyLanguage(lang)}
function setDriftConfigStatus(key,kind,suffix){driftStatusKey=key;driftStatusKind=kind||'';driftStatusSuffix=suffix||'';driftConfigStatusEl.textContent=t(driftStatusKey)+driftStatusSuffix;driftConfigStatusEl.style.color=driftStatusKind==='ok'?'#39d98a':driftStatusKind==='err'?'#ff7b7b':'#8fa1b5'}
...
function renderDriftState(latest){if(!latest)return;lastDriftLatest=latest;driftOnline=true;lastSeq=Math.max(lastSeq,Number(latest.seq||0));stateEnabledEl.textContent=latest.de?t('drift.value.on'):t('drift.value.off');stateActiveEl.textContent=latest.da?t('drift.value.active'):t('drift.value.armed');stateYawErrorEl.textContent=Number(latest.dye||0).toFixed(2);const m=Number(latest.dtm||0);stateThrottleModeEl.textContent=m===2?t('drift.value.spin'):m===1?t('drift.value.pulse'):t('drift.value.pass');driftStateCurrentEl.textContent=t('drift.state.seqPrefix')+latest.seq;}
async function pollDriftData(){try{const r=await fetch('/api/data?since='+lastSeq,{cache:'no-store'});const j=await r.json();if(j&&j.latest)renderDriftState(j.latest)}catch(e){driftOnline=false;driftStateCurrentEl.textContent=t('drift.state.offline')}}
initLanguage();loadDriftConfig();setInterval(pollDriftData,250);
</script>
</body>
</html>
)rawliteral";
...
<body>
<h1 data-i18n="ota.heading">MUS4 HTTP OTA</h1>
<div id="drop">
...
function setStatus(text,c){status.textContent=text;status.style.color=c||'#8fa1b5'}
...
async function upload(f){
  ...
  xhr.addEventListener('load',()=>{
    if(xhr.status===200){setStatus(t('ota.status.success')+xhr.responseText,'#39d98a');setTimeout(()=>location.href='/',3000)}
    else{setStatus(t('ota.status.errorPrefix')+xhr.status+': '+xhr.responseText,'#ff6666')}
  });
  xhr.addEventListener('error',()=>setStatus(t('ota.status.networkError'),'#ff6666'));
