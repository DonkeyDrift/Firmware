...
<div id="networkCard" class="stateCard"><div class="netTabs"><button id="networkApTab" type="button" onclick="setNetworkTab('ap')">AP</button><button id="networkStaTab" type="button" onclick="setNetworkTab('sta')">STA</button></div><button class="gear" onclick="event.stopPropagation();openWifiStaModal()">⚙</button><div class="stateHead">Network</div><div class="stateValue" id="networkValue">--</div><div class="stateSub" id="networkSub">waiting</div><div class="networkMeta"><b>SSID</b><span id="networkSsidValue">--</span><b>IP</b><span id="networkIpValue">--</span></div><span class="stateDot"></span></div>
</div>
...
function updateNetworkCard(s){updateNetworkCard.last=s;const ap=s.ap_ip||'--',sta=s.sta_ip||'0.0.0.0',apSsid=s.ap_ssid||'MUS4-DEBUG',staSsid=s.sta_ssid||'--',clients=s.ap_clients||'0',configured=s.sta_configured==='1',staConnected=s.sta_connected==='1',selected=networkTabPinned?networkTab:(staConnected?'sta':'ap');networkApTab.classList.toggle('active',selected==='ap');networkStaTab.classList.toggle('active',selected==='sta');if(selected==='ap'){networkValue.textContent=ap;networkSub.textContent='clients '+clients;networkSsidValue.textContent=apSsid;networkIpValue.textContent=ap;networkCard.className='stateCard mode0'}else{networkValue.textContent=configured?sta:'disabled';networkSub.textContent=staConnected?'connected':configured?'pending':'not configured';networkSsidValue.textContent=configured?staSsid:'--';networkIpValue.textContent=sta;networkCard.className='stateCard '+(staConnected?'mode0':'driftOff')}if(s.version){versionLabel.textContent=s.version}}
