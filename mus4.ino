...
<div id="voltageCard" class="stateCard"><div class="stateHead">Voltage</div><div class="stateValue" id="voltageValue">--</div><div class="stateSub" id="voltageSub">battery</div><span class="stateDot"></span></div>
</div>
...
<div class="row" style="margin-top:8px"><button onclick="startTubRecording()" id="tubStartBtn">Start Tub</button><button onclick="stopTubRecording()" id="tubStopBtn">Stop Tub</button><button onclick="downloadTubJson()" id="tubDownloadBtn">Download Tub JSON</button></div>
<div class="muted" id="tubMeta">tub ready</div>
...
function cloneTubPoint(p){const keys=['seq','t','thr','str','mode','park','rct','rcs','ch1','ch2','ch3','ch4','ch5','ch6','pt','ps','cur','vol','gz','de','da','dc','gzf'];const out={};for(const k of keys)out[k]=p[k]??0;return out}
function updateTubMeta(){const state=tubRecording?'Recording':'Stopped';tubMeta.textContent=state+': '+tubSamples.length+'/'+TUB_MAX_SAMPLES+' samples';tubStartBtn.disabled=tubRecording;tubStopBtn.disabled=!tubRecording;tubDownloadBtn.disabled=tubSamples.length===0}
function startTubRecording(){tubSamples=[];tubStartedMs=0;tubStoppedMs=0;tubLastSeq=0;tubRecording=true;updateTubMeta();line('[tub] recording started')}
function stopTubRecording(){if(!tubRecording)return;tubRecording=false;tubStoppedMs=tubSamples.length?tubSamples[tubSamples.length-1].t:tubStartedMs;updateTubMeta();line('[tub] recording stopped samples='+tubSamples.length)}
function captureTubPoint(p){if(!tubRecording||!p)return;if([p.ch1,p.ch2,p.ch3,p.ch4,p.ch5,p.ch6].some(v=>v===undefined))return;const seq=Number(p.seq||0);if(seq&&seq===tubLastSeq)return;if(tubSamples.length===0)tubStartedMs=Number(p.t||0);tubSamples.push(cloneTubPoint(p));tubLastSeq=seq;tubStoppedMs=Number(p.t||tubStoppedMs);if(tubSamples.length>=TUB_MAX_SAMPLES){stopTubRecording();line('[tub] sample limit reached')}else updateTubMeta()}
function downloadTubJson(){if(tubSamples.length===0){line('[tub] no samples to download');return}const stopped=tubRecording?(tubSamples[tubSamples.length-1].t||tubStartedMs):tubStoppedMs;const tubPackage={schema:TUB_SCHEMA,source:'mus4-web-console',started_ms:tubStartedMs,stopped_ms:stopped,count:tubSamples.length,samples:tubSamples};const blob=new Blob([JSON.stringify(tubPackage,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='mus4-tub-'+new Date().toISOString().replace(/[:.]/g,'-')+'.json';document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);line('[tub] downloaded samples='+tubSamples.length)}
...
updateTubMeta();refreshStatus();refreshDevMode();refreshWifiSta();setInterval(refreshStatus,5000);setInterval(refreshWifiSta,5000);setInterval(pollLog,1000);connectDataSocket();setTimeout(()=>{if(!dataWsConnected)pollData()},1200);draw();requestAnimationFrame(renderLoop);
</script>
</body>
</html>
)rawliteral";
