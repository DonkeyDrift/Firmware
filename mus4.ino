...
function clearChart(){pointHead=0;pointCount=0;points.fill(null);pendingPoints=[];scrollOffset=0;queuePrimed=false;gapCount=0;latestBackendTime=0;renderTime=0;smoothedDt=16;dataWsPingTimer=0;gridReady=false;draw()}function enterScreenSaver(){screenSaverActive=true;screenSaverStartTime=performance.now();clearChart();dataMeta.textContent='screensaver active'}function exitScreenSaver(){screenSaverActive=false;screenSaverStartTime=0;parkLockedAt=0;ch1Samples=[];clearChart();dataMeta.textContent='screensaver exited'}
