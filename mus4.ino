...
let lastLogSeq=0,lastDataSeq=0,logPaused=false,chartPaused=false,dataPolling=false,drawPending=false,points=[];
...
function clearChart(){points=[];scheduleDraw()}
...
async function pollData(){if(dataPolling)return;dataPolling=true;try{const r=await fetch('/api/data?since='+lastDataSeq);const arr=await r.json();let latest=null;for(const p of arr){lastDataSeq=Math.max(lastDataSeq,p.seq);latest=p;if(!chartPaused)points.push(p)}if(latest)updateState(latest);if(points.length>256)points=points.slice(-256);if(arr.length&&!chartPaused)scheduleDraw();const p=latest||points[points.length-1];dataMeta.textContent=p?'seq='+lastDataSeq+' thr='+p.thr+' str='+p.str+' ch4='+p.ch4+' cur='+p.cur+' gz='+p.gz:'waiting data'}catch(e){dataMeta.textContent='data error: '+e}finally{dataPolling=false;setTimeout(pollData,16)}}
