...
<div id="networkCard" class="stateCard"><div class="netTabs"><button id="networkApTab" type="button" onclick="setNetworkTab('ap')">AP</button><button id="networkStaTab" type="button" onclick="setNetworkTab('sta')">STA</button></div><button class="gear" onclick="event.stopPropagation();openWifiStaModal()">⚙</button><div class="stateHead">Network</div><div class="stateValue copyValue" id="networkValue" onclick="copyNetworkIp()">--</div><div class="stateMeta"><b>SSID</b><span id="networkSsidValue">--</span></div><span class="stateDot"></span></div>
</div>
...
<div class="chartControls"><button onclick="toggleChart()" id="chartBtn">暂停</button><button onclick="clearChart()">清空</button><button onclick="toggleChartFullscreen()" id="chartFullscreenBtn">全屏</button><div class="chartTools"><button onclick="ts()">Tub Start</button><button onclick="te()">Tub Stop</button><button onclick="td()">Tub JSON</button></div></div>
</section>
...
<div id="wifiStaModal" class="modal"><div class="dialog"><h2>STA Wi-Fi 配置</h2><div class="row"><div class="inputWithAction"><input id="staSsid" placeholder="STA SSID"><button id="staSsidSearchBtn" class="iconButton" type="button" onclick="openWifiScanPopover(event)">⌕</button><div id="wifiScanPopover" class="scanPopover"><div id="wifiScanStatus" class="muted">扫描中...</div><div id="wifiScanList"></div></div></div></div><div class="row" style="margin-top:8px"><div class="inputWithAction"><input id="staPassword" type="password" placeholder="Wi-Fi 密码，留空表示开放网络"><button id="staPasswordEye" class="iconButton" type="button" onmousedown="showStaPassword()" onmouseup="hideStaPassword()" onmouseleave="hideStaPassword()" ontouchstart="showStaPassword()" ontouchend="hideStaPassword()">👁</button></div></div><p>注意只能连接2.4G WiFi</p><div class="dialogActions"><button onclick="closeWifiStaModal()">取消</button><button onclick="clearWifiSta()">清除</button><button onclick="saveWifiSta()">保存并连接</button></div></div></div>
...
function toggleChart(){chartPaused=!chartPaused;document.getElementById('chartBtn').textContent=chartPaused?'绘制':'暂停'}
...
document.addEventListener('fullscreenchange',()=>{document.getElementById('chartFullscreenBtn').textContent=document.fullscreenElement===chartPanel?'分屏':'全屏';gridReady=false;draw()});
...
function explainCommandError(t){if(t.includes('PARK_REQUIRED'))return '当前操作需要 Park Locked。请将 CH3/Park 切到锁定状态后重试。';if(t.includes('AUTH_REQUIRED')||t.includes('UNAUTHORIZED'))return '当前操作需要授权。请先 AUTH，或开启 DEV MODE 后重试。';return ''}
...
function renderStaPasswordState(j,force=false){if(!j)return;if(!force&&(document.activeElement===staPassword||staPasswordDirty))return;if(j.connected&&j.password_set){staPassword.value='*';staPassword.type='password';staPasswordPlaceholder=true}else{staPassword.value='';staPassword.type='password';staPasswordPlaceholder=false}staPasswordDirty=false}
...
function selectWifiSsid(ssid){staSsid.value=ssid;closeWifiScanPopover();staPassword.focus()}
function showStaPassword(){if(!staPasswordPlaceholder)staPassword.type='text'}
function hideStaPassword(){staPassword.type='password'}
async function openWifiStaModal(){closeWifiScanPopover();await refreshWifiSta(true);wifiStaModal.classList.add('show')}
function closeWifiStaModal(){closeWifiScanPopover();hideStaPassword();wifiStaModal.classList.remove('show')}
...
async function saveWifiSta(){try{closeWifiScanPopover();const body=new URLSearchParams();body.set('ssid',staSsid.value.trim());if(staPasswordPlaceholder&&!staPasswordDirty)body.set('keep_password','1');else body.set('password',staPassword.value);const r=await fetch('/api/wifi-sta',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body});if(!r.ok){const t=await r.text();showWifiStaFailureModal({ssid:staSsid.value.trim(),last_error_message:t});throw new Error(t)}staPassword.value='';staPasswordPlaceholder=false;staPasswordDirty=false;await refreshWifiSta(true);refreshStatus();await new Promise(resolve=>setTimeout(resolve,1000));await waitWifiStaConnectionResult()}catch(e){line('wifi sta save error: '+e)}}
async function clearWifiSta(){if(!confirm('确认清除并禁用 STA 配置？'))return;try{closeWifiScanPopover();const r=await fetch('/api/wifi-sta/clear',{method:'POST'});if(!r.ok){const t=await r.text();showCommandError(t);throw new Error(t)}staPassword.value='';staPasswordPlaceholder=false;staPasswordDirty=false;await refreshWifiSta(true);refreshStatus();closeWifiStaModal()}catch(e){line('wifi sta clear error: '+e)}}
async function quick(v){cmd.value=v;await sendCmd()}
staPassword.addEventListener('input',()=>{if(staPasswordPlaceholder){staPassword.value=staPassword.value.replace(/^\*/,'');staPasswordPlaceholder=false}staPasswordDirty=true});
