...
static void sampleWifiWebData()
{
    ...
    // [SINE-TEST] 正弦波模拟，仅用于评估曲线绘制效果
    point.throttle = (int16_t)(95.0f * sinf((float)millis() / 400.0f));
    point.steering = (int16_t)(70.0f * sinf((float)millis() / 550.0f + 1.0f));
    point.gyroZ = 3.5f * sinf((float)millis() / 300.0f + 2.0f);
...
function clearChart(){pointHead=0;pointCount=0;points.fill(null);pendingPoints=[];scrollOffset=0;queuePrimed=false;gapCount=0;latestBackendTime=0;renderTime=0;smoothedDt=16;dataWsPingTimer=0;gridReady=false;draw()}
